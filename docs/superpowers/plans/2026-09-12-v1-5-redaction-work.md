...
用户追加要求“完成剩下的全部功能”后，原可选项已纳入本轮必做范围。下方首次交付状态为历史记录；最新功能说明与验证边界见 [开发交付说明](../../v1.5-development-notes.md)。

| 优先级 | 原清单事项 | 本轮补充 |
|---|---|---|
| P0 | 加密草稿恢复 | 批次草稿独立作用域、删除原件时清理全部关联草稿 |
| P1 | 重复文字批量处理 | 同行多个匹配不再提前停止识别 |
| P1 | PDF 待处理导航 | 保留已实现功能 |
| P1 | 分享扩展 | 保留已实现功能，真机签名验收仍待执行 |
| P1 | 视频手动补漏 | 绘制/确认时间/重画/删除；预览与导出共用处理 |
| P1 | 导出摘要 | 补全实际音频模式与容器元数据检查项数 |
| P2 | 图片批处理 | 统一规则、逐张复核、暂停恢复、失败重试、固定导出 ID |
| P2 | OCR 手势 | 滑选、放大镜、手柄、列表选择及一次撤销 |
| P2 | 视频人物管理 | 带编号的跟踪片段选择、用户确认归组、统一效果 |
| P2 | 首次使用流程 | 单文件快捷编辑入口、可关闭的人工复核引导 |
| P2 | 文档同步 | 中英文 README 移除“逐帧检测”承诺并说明动态采样 |
